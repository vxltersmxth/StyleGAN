    ...
    def train(self, dataset, num_workers, epochs, batch_sizes, fade_in_percentage, logger, output,
        ...
        try:  
            for current_depth in range(start_depth, self.depth):
                ...
                # TODO
                logger.info(f'batch size {batch_sizes[current_depth]}')
                ...
                for epoch in range(1, epochs[current_depth] + 1):
                    ...
                    for (i, batch) in enumerate(data, 1):
                        ...
                        if i % int(total_batches / feedback_factor + 1) == 0 or i == 1:
                            ...
                            with torch.no_grad():
                                self.create_grid(
                                    samples=self.gen.cpu()(fixed_input.cpu(), current_depth, alpha).detach() if not self.use_ema
                                    else self.gen_shadow.cpu()(fixed_input.cpu(), current_depth, alpha).detach(),
                                    scale_factor=int(
                                        np.power(2, self.depth - current_depth - 1)) if self.structure == 'linear' else 1,
                                    img_file=gen_img_file,
                                )
                            
                            self.gen.cuda() if not self.use_ema else self.gen_shadow.cuda()
                    ...
                    if epoch % checkpoint_factor == 0 or epoch == 1 or epoch == epochs[current_depth] or epoch%4:
