...
def interpolate_points(p1, p2, n_steps=10):
	...
	ratios = linspace(p1, p2, num=n_steps).tolist()
	# linear interpolate vectors
	# vectors = list()
	# for ratio in ratios:
	# 	v = slerp(ratio, p1, p2)
	# 	vectors.append(v)
	return asarray(ratios)
...
def main(args):
    ...
    gen = Generator(resolution=opt.dataset.resolution,
                    ...
                    structure=opt.structure, style_mixing_prob = 0,
                    **opt.model.gen)
    ...
    out_depth = int(np.log2(opt.dataset.resolution // 2)) - 2
    out_depth = 6

    points = generate_latent_points(latent_size, 2)
    number =  args.num_samples+1
    points[0] = gen.truncation.avg_latent.numpy()     
    if args.input is None:
        print("Generating scale synchronized images ...")
        for img_num in tqdm(range(1, number)):
            # generate the images:
            with torch.no_grad():
                number= 24 * 2
                tpoint = points[1]
                interpolated = interpolate_points(points[0], points[1], number)
                point = torch.randn(1, latent_size)
                for i in range(interpolated.shape[0]):
                    
                    point[0] = torch.from_numpy(interpolated[i])
                    ...
                    save_image(ss_image, os.path.join(save_path, str(img_num)+ '_' + str(i) + ".png"))
